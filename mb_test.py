...
epochs_test = [10]
mini_batch_size_test = np.arange(201, 321, 40).tolist()
learning_rate_test = [6]
...
for ite in enumerate(iterations):
    for mbs_idx, mbs in enumerate(mini_batch_size_test):
        ...
        acc_valid, epoch_count = net.SGD(X_train, Y_train, X_validation, Y_validation, 10, mbs, 6, 0.0001)
        ...
        df = df.append({"# of hidden layers": 30, 
                       ...
                       ignore_index=True)
         # sanity check
        percentage += (1.0 / len(mini_batch_size_test)/len(iterations))
        print("Progress: ", '{0:.2f}'.format(percentage * 100), "%")

# name your file
csv_file = 'mini_batch_test_results.csv'
